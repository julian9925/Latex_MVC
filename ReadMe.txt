...
至於以 example_ 為首的幾個檔案，則是用來做示範用的，看完後可以丟棄:
	...
	example_fig.bb  示範論文 png 圖檔在 latex+dvipdfmx 工作流程時所需的原圖尺寸檔
	...
	example_macro_demo.pdf 示範了處理插圖的自訂巨集的使用方式。此自訂巨集與論文格式檔無關，定義在 npc_macros.tex 裏。如果要使用，可以在文稿裏載入。細節可閱讀 my_chapters.tex 裏的註解。
...
yzu_backpages.tex 關於參考文獻所使用的格式 (預設是 ieeetr.bst)，以及附錄、自傳頁面的格式。
